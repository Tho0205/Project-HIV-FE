...
const PatientMedicalRecordPage = () => {
  ...
  const loadAllData = useCallback(async () => {
    ...
    try {
      ...
      const examinationsPromise = hivExaminationService.getPatientHistory(patientId)
      ...
      const protocolsPromise = CustomArvProtocolsService.getPatientProtocolHistory(patientId)
        .catch((err) => {
  ...
  const getStatusStyle = (status) => {
    ...
    if (statusLower.includes('active')) return { ...baseStyle, backgroundColor: "#dcfce7", color: "#15803d" };
    if (statusLower.includes('completed')) return { ...baseStyle, backgroundColor: "#dbeafe", color: "#1e40af" };
    if (statusLower.includes('inactive')) return { ...baseStyle, backgroundColor: "#fef9c3", color: "#a16207" };
    if (statusLower.includes('cancelled')) return { ...baseStyle, backgroundColor: "#fee2e2", color: "#b91c1c" };
  ...
  const filteredExaminations = examinations.filter((exam) => {
    const matchesSearch = 
      (exam.doctorName && exam.doctorName.toLowerCase().includes(searchTerm.toLowerCase())) ||
    ...
    else if (filterStatus === "pending") matchesFilter = exam.status !== "ACTIVE";
  ...
  const filteredProtocols = protocolHistory.filter((protocol) => {
    const matchesSearch = 
      (protocol.name && protocol.name.toLowerCase().includes(searchTerm.toLowerCase())) ||
    ...
    else if (filterStatus === "inactive") matchesFilter = protocol.status !== "ACTIVE";

    return matchesSearch && matchesFilter;
  });

  // Pagination logic
  const currentData = mainView === "examinations" ? filteredExaminations : filteredProtocols;
      ...
      <div className="container">
        ...
        <section className="profile">
          <div className="card" style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', padding: '3rem 0' }}>
            ...
            <span style={{ marginLeft: '0.75rem', color: '#4b5563' }}>Đang tải dữ liệu y tế...</span>
          </div>
        </section>
      </div>
    );
      ...
      <div className="container">
        ...
        <section className="profile">
          <div className="card" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '3rem 0', color: '#dc2626' }}>
            ...
            <span>{error}</span>
            <button onClick={loadAllData} className="btn-green" style={{ marginTop: '1rem', padding: '0.5rem 1rem' }}>
              Thử lại
            </button>
          </div>
        </section>
      ...
      <section className="profile">
        ...
        {patientInfo && (
          <div className="card profile-header">
            ...
            <div className="profile-info">
              ...
              <p>
                {patientInfo.gender === "Male" ? "Nam" : patientInfo.gender === "Female" ? "Nữ" : "Chưa cập nhật"}
              </p>
            </div>
          </div>
        )}
        ...
        <div className="card">
          ...
          <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1.5rem', borderBottom: '2px solid #e2e8f0' }}>
            <button 
              onClick={() => { setMainView("examinations"); setCurrentPage(1); }}
              style={{
                padding: '1rem 1.5rem',
                background: 'none',
                border: 'none',
                ...
                cursor: 'pointer',
                borderBottom: mainView === "examinations" ? '3px solid #3b82f6' : '3px solid transparent',
            ...
            <button 
              onClick={() => { setMainView("protocols"); setCurrentPage(1); }}
              style={{
                padding: '1rem 1.5rem',
                background: 'none',
                border: 'none',
                ...
                cursor: 'pointer',
                borderBottom: mainView === "protocols" ? '3px solid #3b82f6' : '3px solid transparent',
                transition: 'all 0.3s ease'
              }}
            >
          ...
          <div style={{ display: 'flex', flexDirection: 'row', gap: '1rem', marginBottom: '1.5rem' }}>
            <div style={{ flex: '1', position: 'relative' }}>
              ...
              <input
                type="text"
                placeholder={mainView === "examinations" ? "Tìm kiếm theo bác sĩ hoặc kết quả..." : "Tìm kiếm theo tên hoặc mô tả phác đồ..."}
                value={searchTerm}
              ...
              >
                {mainView === "examinations" ? (
                  <>
                    ...
                    <option value="completed">Đã hoàn thành ({examinations.filter(e => e.status === "ACTIVE").length})</option>
                    <option value="pending">Đang xử lý ({examinations.filter(e => e.status !== "ACTIVE").length})</option>
                  </>
                ) : (
                  <>
                    <option value="all">Tất cả ({protocolHistory.length})</option>
                    <option value="active">Đang áp dụng ({protocolHistory.filter(p => p.status === "ACTIVE").length})</option>
                    <option value="inactive">Không áp dụng ({protocolHistory.filter(p => p.status !== "ACTIVE").length})</option>
                  </>
                )}
              </select>
            </div>
          ...
          <div style={{ marginTop: '1rem' }}>
            {currentItems.length === 0 ? (
              ...
              currentItems.map((item) => (
                <div key={mainView === "examinations" ? item.examId : item.customProtocolId} className="card" style={{ padding: '1.5rem', marginBottom: '1rem' }}>
                  {mainView === "examinations" ? (
                    // Examination Card
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '1rem' }}>
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ marginTop: '0.25rem', fontSize: '0.875rem', color: '#4b5563' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                            ...
                            <User style={{ width: '1rem', height: '1rem', marginLeft: '0.5rem' }} />
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '0.75rem', marginTop: '1rem' }}>
                          {item.cd4Count && (
                            <div style={{ padding: '0.5rem', backgroundColor: '#dcfce7', borderRadius: '0.5rem', textAlign: 'center' }}>
                              <div style={{ fontSize: '0.75rem', color: '#166534' }}>CD4</div>
                              <div style={{ fontWeight: '700', color: '#059669' }}>{item.cd4Count} tế bào/mm³</div>
                            </div>
                          )}
                          {item.hivLoad && (
                            <div style={{ padding: '0.5rem', backgroundColor: '#fee2e2', borderRadius: '0.5rem', textAlign: 'center' }}>
                              <div style={{ fontSize: '0.75rem', color: '#dc2626' }}>Tải lượng HIV</div>
                              <div style={{ fontWeight: '700', color: '#dc2626' }}>{item.hivLoad} copies/ml</div>
                            </div>
                          )}
                        </div>
                      </div>
                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '0.5rem' }}>
                        <span style={getStatusStyle(item.status)}>
                          {item.status === "ACTIVE" ? "Hoàn thành" : item.status}
                        </span>
                      </div>
                    </div>
                  ) : (
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '1rem' }}>
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ marginTop: '0.25rem', fontSize: '0.875rem', color: '#4b5563' }}>
                          {item.baseProtocolName && (
                            <div style={{ marginBottom: '0.25rem' }}>
                              <span style={{ fontWeight: '500' }}>Dựa trên:</span> {item.baseProtocolName}
                            </div>
                          )}
                          {item.description && (
                        ...
                        {/* ARV Details */}
                        <div style={{ marginTop: '1rem' }}>
                          <h5 style={{ fontSize: '0.875rem', fontWeight: '600', color: '#475569', marginBottom: '0.5rem' }}>
                            Danh sách thuốc ARV:
                          </h5>
                          {item.details && item.details.length > 0 ? (
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                              {item.details.slice(0, 3).map((detail, index) => (
                                <div key={index} style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 2fr', gap: '0.5rem', padding: '0.5rem', backgroundColor: '#f8fafc', borderRadius: '0.5rem', fontSize: '0.875rem' }}>
                                  <span style={{ fontWeight: '500', color: '#1e293b' }}>{detail.arvName}</span>
                                  <span style={{ color: '#059669', textAlign: 'center' }}>{detail.dosage}</span>
                                  <span style={{ color: '#64748b' }}>{detail.usageInstruction}</span>
                                </div>
                              ))}
                            ...
                            <p style={{ color: '#94a3b8', fontStyle: 'italic', margin: 0 }}>Chưa có thuốc ARV nào</p>
                          )}
                        </div>
                      </div>
                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '0.5rem' }}>
                        <span style={getStatusStyle(item.status)}>
                          {item.status === "ACTIVE" ? "Đang áp dụng" : item.status}
                        </span>
                      </div>
                    </div>
                  )}
        ...
        <div style={{
          ...
          <div style={{ textAlign: 'center', padding: '1rem', background: 'rgba(255, 255, 255, 0.1)', borderRadius: '1rem' }}>
            <div style={{ fontSize: '2rem', fontWeight: '800', marginBottom: '0.5rem' }}>{examinations.length}</div>
            <div style={{ fontSize: '1rem', opacity: '0.9', fontWeight: '500' }}>Lần xét nghiệm</div>
          </div>
          <div style={{ textAlign: 'center', padding: '1rem', background: 'rgba(255, 255, 255, 0.1)', borderRadius: '1rem' }}>
            <div style={{ fontSize: '2rem', fontWeight: '800', marginBottom: '0.5rem' }}>{protocolHistory.length}</div>
            <div style={{ fontSize: '1rem', opacity: '0.9', fontWeight: '500' }}>Phác đồ ARV</div>
          </div>
          <div style={{ textAlign: 'center', padding: '1rem', background: 'rgba(255, 255, 255, 0.1)', borderRadius: '1rem' }}>
            <div style={{ fontSize: '2rem', fontWeight: '800', marginBottom: '0.5rem' }}>
              {protocolHistory.filter(p => p.status === "ACTIVE").length}
            </div>
            <div style={{ fontSize: '1rem', opacity: '0.9', fontWeight: '500' }}>Phác đồ đang áp dụng</div>
          </div>
        </div>
      </section>
    </div>
